...
public class MusicPlayerFragment extends Fragment {
     ...
     * 加载专辑封面图片
     *
     * <p>优先使用 MusicItem 中缓存的封面数据，提取失败时显示默认封面。
     *
     * @param item 音乐项
     */
    private void loadAlbumArt(MusicItem item) {
        ...
        if (item.albumArt != null && item.albumArt.length > 0) {
            try {
                // 使用 MusicItem 中缓存的封面数据
                Bitmap bitmap = BitmapFactory.decodeByteArray(item.albumArt, 0, item.albumArt.length);
                if (bitmap != null) {
                    ivAlbumArt.setImageBitmap(bitmap);
                    return;
                }
            } catch (Exception e) {
                AppLog.d(TAG, "Failed to decode album art");
            }
        }
        // 封面提取失败或无封面时显示默认封面
        ivAlbumArt.setImageResource(R.drawable.ic_launcher_playstore);
