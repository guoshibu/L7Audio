...
public class MainActivity extends BaseActivity {
    ...
    /**
     * 配置变更时调用（横竖屏切换、屏幕尺寸变化等）
     *
     * <p>由于 AndroidManifest 配置了 configChanges，Activity 不会重建，
     * 但 Fragment 的布局需要重新加载以适配横屏/竖屏布局。
     * 通过重新 replace 当前功能 Fragment，触发 onCreateView 重新加载对应方向的布局。
     *
    ...
    public void onConfigurationChanged(android.content.res.Configuration newConfig) {
        ...
        AppLog.d(TAG, "onConfigurationChanged: orientation=" + newConfig.orientation);

        // 重新加载当前功能页面的 Fragment，以刷新布局（加载 layout-land 或 layout-port）
