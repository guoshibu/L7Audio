 ...
 * </ul>
 *
 * <p>架构：使用 TTSViewModel 管理数据，TTSRepository 负责持久化。
 * 采用 MVVM 架构，通过 LiveData 观察数据变化并自动刷新 UI。
 *
...
public class TTSFragment extends BaseFragment {
    ...
    /**
     * TTS UI 项（包含播放状态）
     * 注意：这是 UI 层的数据模型，不同于 data.model.TTSItem（持久层）
     */
     ...
     * 初始化视图控件。
     *
     * <p>查找所有 UI 控件，初始化 TTS 管理器并设置播放进度监听器，
     * 初始化数据列表和按钮点击事件。
     *
     ...
     * 初始化数据。
     *
     * <p>初始化 ViewModel 并观察 TTS 列表数据的变化，
     * 数据变化时自动刷新 UI。LiveData 具有生命周期感知能力，
     * 可避免内存泄漏和无效的 UI 更新。
     */
    @Override
    protected void initData() {
        // 初始化 ViewModel（必须在 UI 线程）
        viewModel = new ViewModelProvider(this).get(TTSViewModel.class);

        // 观察 TTS 列表 LiveData，加载后刷新视图
     ...
     * Fragment 视图销毁时调用。
     *
     * <p>清理 TTS 进度监听器，置空 View 引用，防止内存泄漏。
     */
    @Override
     ...
     * 初始化事件监听器。
     *
     * <p>TTSFragment 的监听器已在 initViews 中设置，
     * 此方法仅作为基类抽象方法的空实现。
     */
    @Override
    ...
    /** 添加新的 TTS 条目（通过 ViewModel → Repository 持久化） */
    ...
    /** 显示悬浮窗列表编辑器（使用 FloatingWindowConfig 替代 AppConfig） */
     ...
     * Fragment 销毁时的回调。
     *
     * <p>停止当前正在播放的 TTS，释放语音合成资源，
     * 避免 Fragment 销毁后仍在后台播放。
     */
    @Override
