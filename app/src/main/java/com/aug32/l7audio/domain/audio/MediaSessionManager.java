...
public class MediaSessionManager {
            ...
            if (item != null) {
                ...
                if (item.albumArt != null && item.albumArt.length > 0) {
                    try {
                        ...
                        AppLog.w(TAG, "Failed to decode album art bitmap: " + e.getMessage());
                    }
                }
     ...
     * 获取 SessionToken
     *
     * @return MediaSession.Token 实例，可能为 null
     */
    @Nullable
     ...
     * 释放 MediaSession 资源
     *
     * <p>应用退出时调用，释放 MediaSession 相关资源。
     */
    public void release() {
        if (mediaSession != null) {
            try {
                ...
                AppLog.e(TAG, "Failed to release MediaSession", e);
            }
        }
    }
}
