...
public class MusicPlayerManager {
     ...
     * 扫描音乐信息扩展类（兼容旧接口）
     *
     * <p>包含音乐文件的基本信息，用于从 MediaStore 扫描结果创建音乐项。
     */
     ...
     * 设置随机播放模式是否启用
     *
     * <p>启用时切换为随机模式，关闭时如果当前是随机模式则切换为列表循环。
     *
     * @param enabled true 启用随机模式，false 关闭随机模式
     */
    public void setShuffleModeEnabled(boolean enabled) {
        if (enabled) {
            playlistManager.setRepeatMode(REPEAT_MODE_SHUFFLE);
        } else {
            // 只有当前是随机模式时才切换到列表循环，避免覆盖用户设置的其他模式
