...
public class FloatingWindowService extends Service {
    ...
    private void showFloatingBall() {
        if (floatingBallView != null) {
            AppLog.d(TAG, "Floating ball already exists, skipping");
            return;
        }
        AppLog.d(TAG, "Showing floating ball");
    ...
    private void applyThemeToFloatingBall() {
        ...
        AppLog.d(TAG, "Applying theme to floating ball, isDark=" + isDarkTheme());
    ...
    private void applyThemeToListView() {
        if (floatingListView == null) {
            AppLog.d(TAG, "applyThemeToListView: floatingListView is null");
            return;
        }

        boolean isDark = isDarkTheme();
        AppLog.d(TAG, "Applying theme to list view, isDark=" + isDark);
    ...
    private void resetAutoHideTimer() {
        if (handler != null && autoHideRunnable != null) {
            ...
            handler.postDelayed(autoHideRunnable, appConfig.getFloatingWindowAutoHideTimeoutSec() * 1000);
        }
    }
    ...
    private void setupListView() {
        ...
        sbAutoHide.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override
            public void onStopTrackingTouch(SeekBar seekBar) {
                appConfig.setFloatingWindowAutoHideTimeoutSec(5 + seekBar.getProgress());
                resetAutoHideTimer();
            }
        });
    ...
    /** 切换车外喊话的开启/关闭，通过 MicOutputController 统一管理 */
    private void toggleAnnouncement() {
        AppLog.d(TAG, "toggleAnnouncement called, current isAnnouncing=" + isAnnouncing);
        MicOutputController.getInstance().toggle(true);
    }
    
    /** 根据当前是否在喊话状态更新喊话按钮的 UI */
    private void updateAnnouncementButton() {
        if (announcementBtn == null) {
            AppLog.d(TAG, "updateAnnouncementButton: announcementBtn is null");
            return;
        }

        AppLog.d(TAG, "updateAnnouncementButton: isAnnouncing=" + isAnnouncing);
        if (isAnnouncing) {
    ...
    private void playTTS(String text, Button clickedBtn) {
        AppLog.d(TAG, "playTTS: textLen=" + (text != null ? text.length() : 0) + " isAnnouncing=" + isAnnouncing);
        ...
        try {
            if (audioServiceLocator.getTTSManager() != null) {
                int externalUsage = outputManager.getExternalAudioUsage();
